...
result_with_content_path = r"../fin_data/scrape/results_with_content"
...
def get_one_content(x, file_name):
    ...
    while not ok:
        try:
            ...
            if response.status_code == 200:
                ...
                scrip_texts = res.xpath("//script//text()")
                if len(scrip_texts) >= 2:
                    res = res.xpath("//script[2]//text()")[0]
                    res = json.loads(res[17:])
                    arranged_res = [res[keyword] for keyword in new_columns]
                    return_res = pd.Series(arranged_res)
                    ok = True
                    progress[file_name] += 1
                    if progress[file_name] % 20 == 0:
                        print(file_name, progress[file_name])
                        # print(res)
                    return return_res
                else:
                    print('Expected second script tag not found')
                    print(scrip_texts)
                    return pd.Series(['-' for keyword in new_columns])
...
def get_content(file_name):
    ...
    df[new_columns] = df.apply(
        lambda x:get_one_content(x, file_name), 
